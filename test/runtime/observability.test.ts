...
describe("runtime observability", () => {
  ...
  test("keeps literal think-block child reasoning out of source researcher parent output", async () => {
    const harness = await createHarness("trace-subagent-think-block-no-leak", true)
    const childPrompt = "Read README.md for a source note."
    const childReasoning = "Need to inspect README before writing the source note."
    const childOutput = "proposed type: files\ntitle: README\nkey excerpts: Neo Coworker"
    const receivedBodies: unknown[] = []
    const started = startPromptRun({
      repository: harness.repository,
      service: harness.service,
      sessionId: harness.session.id,
      runId: "run_trace_subagent_think_block_parent",
      messageId: "message_trace_subagent_think_block_parent",
      prompt: "Delegate README source collection through the source researcher.",
    })
    const provider = createOrchestrationModelPort(createOpenAICompatibleModelProvider({
      model: "MiniMax-M2.7",
        ...
        return (async function* () {
          switch (receivedBodies.length) {
            case 1:
              ...
              return
            case 2:
              yield createOpenAICompatibleChunk({ content: `<think data-source="child">${childReasoning}</think>` })
              yield createOpenAICompatibleChunk({ content: childOutput })
              return
            case 3: {
              const requestText = JSON.stringify(body)

              expect(requestText).toContain(childOutput)
              expect(requestText).toContain(childReasoning)
              expect(requestText).not.toContain("<think")
              expect(requestText).not.toContain("</think>")
    ...
    expect(parentAgentResult?.text).toBe(childOutput)
    expect(parentAgentResult?.text).not.toContain(childReasoning)
    expect(parentAgentResult?.text).not.toContain("<think")
    expect(parentVisibleText).toContain(childOutput)
    expect(parentVisibleText).not.toContain(childReasoning)
    expect(parentVisibleText).not.toContain("<think")
    expect(childVisibleText).toContain(childOutput)
    expect(childVisibleText).not.toContain(childReasoning)
    expect(childVisibleText).not.toContain("<think")
  })
