...
describe("chat command", () => {
  ...
  test("cancels a permission-blocked local chat session on exit and does not recover on a fresh entrypoint", async () => {
    ...
    try {
      expect(secondStorage.repository.runs.get(firstRun.id).status).toBe("cancelled")
    ...
    expect(secondOutput.join("")).not.toContain("permission> Allow")
    expect(secondOutput.join("")).not.toContain("Write finished.")
  })
